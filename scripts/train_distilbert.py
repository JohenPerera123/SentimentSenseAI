...
# Configurations
MAX_LENGTH = 128
TRANSFORMER_TRAIN_SIZE = 5000 if torch.cuda.is_available() else 500
TRANSFORMER_VAL_SIZE = 1000 if torch.cuda.is_available() else 100
TRANSFORMER_TEST_SIZE = 5000 if torch.cuda.is_available() else 500
BATCH_SIZE = 16
EPOCHS = 2
LEARNING_RATE = 2e-5
...
def train_and_evaluate_distilbert(dataset_name: str, train_df, val_df, test_df):
    print(f"--- Fine-Tuning DistilBERT on {dataset_name} ---")
    
    # Subsetting
    train_df = train_df.sample(n=min(TRANSFORMER_TRAIN_SIZE, len(train_df)), random_state=42)
    val_df = val_df.sample(n=min(TRANSFORMER_VAL_SIZE, len(val_df)), random_state=42)
    # For final test evaluation we can use more or all, but let's keep reasonable
    test_df = test_df.sample(n=min(TRANSFORMER_TEST_SIZE, len(test_df)), random_state=42)
    ...
    output_dir = f"./distilbert_results_{dataset_name.lower()}"
    ...
    training_args = TrainingArguments(
        ...
        logging_dir=f"./logs_{dataset_name.lower()}",
        seed=42,
    ...
    try:
        ...
        print("Hardware limitation likely hit. Will skip this training gracefully.")
        return None
        
    print("Saving model...")
    save_dir = os.path.join("models", "distilbert", dataset_name.lower())
    ...
    plt.ylabel('Actual')
    plt.title(f'Confusion Matrix: DistilBERT on {dataset_name}')
    reports_dir = os.path.join("reports", "figures")
    os.makedirs(reports_dir, exist_ok=True)
    plt.savefig(os.path.join(reports_dir, f"{dataset_name.lower()}_distilbert_confusion_matrix.png"))
    plt.close()
    
    # Error analysis
    print(f"Generating error analysis for {dataset_name}...")
    ...
    errors_sample.to_csv(os.path.join("reports", f"distilbert_errors_{dataset_name.lower()}.csv"), index=False)
...
def main():
    os.makedirs(os.path.join("models", "distilbert"), exist_ok=True)
    ...
    imdb_res = train_and_evaluate_distilbert("IMDb", imdb_train, imdb_val, imdb_test)
    twitter_res = train_and_evaluate_distilbert("Twitter", twitter_train, twitter_val, twitter_test)
    
    all_results_list = [res for res in [imdb_res, twitter_res] if res is not None]
    if len(all_results_list) > 0:
        all_results = pd.concat(all_results_list, ignore_index=True)
        # Save isolated results
        all_results.to_json("reports/distilbert_test_results.json", orient='records', indent=4)
        ...
        print("DistilBERT Training and Evaluation complete!")
    else:
        print("DistilBERT training blocked by local hardware constraints.")
